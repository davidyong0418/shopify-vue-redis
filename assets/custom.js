...
var HeaderTop = '';
function stuckNav(){
  if ($j(".header-design11").length > 0) {
    HeaderTop = $j('.header-top-bar-wrapper').offset().top+$j('.header-top-bar-wrapper').innerHeight();
    $j(window).scroll(function(){
      checkStickyPosition();
    });
    $j(window).resize(function(){
...
function checkStickyPosition(){
  $j(window).scrollTop() > HeaderTop ? $j('.header-wrapper').addClass('header-fixed') : $j('.header-wrapper').removeClass('header-fixed');
}
